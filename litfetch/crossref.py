...
async def fetch_work(doi: str, *, http: _http.Http, mailto: str = _http.CONTACT_EMAIL) -> dict | None:
    """Return the Crossref ``message`` object for ``doi``, or ``None``.

    Args:
        doi: The DOI to look up.
        http: The :class:`~litfetch._http.Http` to issue the request on.
        mailto: Identifies the caller for Crossref's polite pool.
    ...
    """
    try:
        resp = await http.get(f'{_CROSSREF_BASE}/{_doi.encode_doi_path(doi)}', params={'mailto': mailto})
